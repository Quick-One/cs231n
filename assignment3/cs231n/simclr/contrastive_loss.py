...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2*N):
            if j != k:
                denominator += torch.exp(sim(z_k, out[j]) / tau)
        
        loss_k = -torch.log(numerator / denominator)

        numerator = torch.exp(sim(z_k_N, z_k) / tau)
        denominator = 0
        for j in range(2*N):
            if j != k+N:
                denominator += torch.exp(sim(z_k_N, out[j]) / tau)
        loss_k_N = -torch.log(numerator / denominator)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = out_left * out_right
    pos_pairs = torch.sum(pos_pairs, dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    magnitude = torch.linalg.norm(out, dim=1, keepdim=True)
    sim_matrix = sim_matrix / (magnitude @ magnitude.T)
